...
## 2026-09-06 — freewebsiteco.com live; contact placeholders gated

- **DNS pointed** at GoDaddy (A @ 76.76.21.21, CNAME www → cname.vercel-dns.com; GoDaddy's "WebsiteBuilder Site" A record removed). Vercel issued SSL; www 308s to the apex (set via the Vercel project-domains API). The old `signalworks-hall-hub.vercel.app` alias still sits behind Vercel SSO because Deployment Protection intercepts before redirects; a host-based 308 to freewebsiteco.com is in `vercel.json` for whenever protection is turned off.
- **Discovery:** Vercel's Standard Protection only guards `*.vercel.app` URLs, so the custom domain is public with protection still ON. That answers "make it public" without touching the security setting — preview deployments stay private.
- **Consequence → `CONTACT_READY`** in `src/config.ts`: because real visitors can now land, every `*_PLACEHOLDER` contact value is hidden sitewide (footer, /contact/, /privacy/ deletion line, JSON-LD email/telephone/streetAddress, and the intake wizard's disabled-state email fallback). Each spot shows honest interim copy ("being set up on the new domain" / "intake opens in the next few days"). Filling `CONTACT` flips everything on with no other edit. `npm run check:launch` now passes.

## 2026-09-06 — Dry run, research pass, and the process gaps both found

- **Client dry run (Ridgeline HVAC, fictional).** Full build → deploy → handoff packet in **11 minutes of build time**; live at ridgelinehvac-demo.vercel.app, repo `~/client-sites/ridgelinehvac-demo`. Two independent audits produced 39 findings (1 blocker, 15 major). Write-up: `docs/dry-run-findings.md`. The build was never the risk; the process around it was.
- **`docs/intake-questions.md` created** — the runbook, the free-build agreement, and the GBP checklist all referenced "intake answers" that were never defined anywhere. Now 30 questions in five sections, mapped to config fields and runbook steps.
- **Runbook updated** with what the dry run exposed: Web3Forms named as the default form service with the account created *in the client's name*; a revision-round section (previously the sequence went build → launch with no client approval step); license-number rule (California contractors must show a CSLB number in advertising — "licensed and insured" never renders without one); address only when customers visit; a privacy page on every client site (CalOPPA, since every client site has a form); care-plan handoff needs its own doc set; a list of templates still to write.
- **Hosting is now a blocking decision, not a preference.** Vercel's fair-use terms restrict Hobby to non-commercial personal use, and freewebsiteco.com is itself commercial. Brief with citations and both migration paths: `docs/hosting-lane-decision.md`. Recommendation is Cloudflare Workers Free ($0, business use allowed), but it moves nameservers off GoDaddy, so it waits on Tyler.
- **Research deliverables added** (all fetched and cited, not recalled): `docs/gbp-setup-sheet.md` (note: Google removed public Q&A in Dec 2025; service area caps at 20 entries and rejects states), `docs/insurance-quote-cheatsheet.md` (NAICS 541511; ~$50–140/mo for GL + E&O), `docs/marketing/la-networking-options.md`, `docs/marketing/handle-availability.md` (Instagram @freewebsiteco is taken), `docs/marketing/prospect-list.md` (15 found, each independently re-verified, 1 rejected for having a decent site).
- **`docs/attorney-review-packet.md` + PDF** — 15 numbered questions covering entity choice, how to describe the Long Hall Financial relationship now that the business must stay separable, liability block, IP assignment, California Automatic Renewal Law, Stripe Connect disclosure, the 60-day bug promise as warranty exposure, SMS consent, and CalOPPA.

## 2026-09-06 — Name research after finding freewebsiteco.com.au

Full write-up: `docs/name-position.md`. Decision: **keep the name, never file for it, never defend it, build the sellable asset elsewhere.**

- The Australian site is a Sunshine Coast **sole trader** (Callum Ray Gracie, ABN 70 955 241 477) with an Australian *business name* registration only — no trademark anywhere, in Australia or the US. Trademark rights are territorial; his operation is AUD-priced, Queensland-governed, with no US-facing content. Low risk.
- **His model is the opposite of ours on the point we sell hardest.** His terms: client licenses the code, he retains ownership, site taken down on cancellation, $800 to buy the source, hosting control "non-negotiable", 6-month minimum with a 50% early-termination fee. That is a competitive advantage for us, not a threat.
- **THE FREE WEBSITE GUYS** (US serial 97059883, Class 042, live, US company, same business model since 2020) is the closest real US exposure. Attorney question 16.
- **freewebsiteco.com had a prior occupant**: a business trading as "The Free Website Company" up to at least Oct 2025, archived title verified, operator apparently India-based. Search engines may carry the old association for a while; Search Console re-indexing handles it. Attorney question 18.
- **freewebsitecompany.com** launched ~May 2026, near-identical offer, trading visibly as "Chroma Marketing Company". Attorney question 17.
- **The name is descriptive** and almost certainly unregistrable on the Principal Register; every other US mark in the "FREE WEBSITE ___" family is dead. Nobody owns it — which means nobody can stop us and we cannot stop anyone. This is the reverse of the Signalworks problem, and the rename risk is lower than it was there.
- **Research quality note:** the first research pass produced 19 claims that failed independent verification, including a fabricated evidence bundle for one site. Only re-verified facts are recorded here.

## 2026-09-06 — Five documents drafted, audited, corrected, re-audited

Written to unblock tasks that could not start without them. Each was drafted, checked twice (content rules + external facts), corrected (173 fixes applied, 29 rejected as unsourced), re-audited, and the 12 surviving must-fix items applied by hand.

- **`docs/lead-services-agreement.md`** — the missing signable one-pager for Lead Capture ($297) and Reputation Engine ($197). Until now those two had no agreement while the site applied a cancel-anytime, 5-business-day-handoff promise to them. Eight decisions are bracketed rather than invented: setup fee, message/minute allowance, support reply time and monthly report (currently Care Plan entitlements that nothing attaches to these services), number-porting routes, export scope, and data retention. It also flags a live contradiction: `SERVICE_LADDER` says a review request goes out "after every job" while the trigger is a job the client marks complete — fix the config blurb, not the agreement.
- **`docs/marketing/prospect-outreach.md`** — email and phone openers for all 14 verified prospects, each resting on a specific verified fact. Ten of the fourteen email closers offer the one-page terms, which still says DRAFT, so the attorney pass gates them.
- **`docs/marketing/post-variants.md`** — the why-free post for five platforms plus three one-line explanations, every version carrying the confirmed-slot, hosting-cost and five-page qualifiers.
- **`docs/money-and-admin-guide.md`** — EIN, bank account, LA BTRC and bookkeeping as cited step-by-step guides in dependency order. Deliberately does not answer which entity reports what for Stripe: under Connect Standard the client owns their own 1099-K and the platform side is fee income. That goes to the CPA.
- **`docs/capacity-model.md`** — arithmetic from stated assumptions: roughly 5–23 hours of Tyler's time per free build and ~3 weeks elapsed intake-to-live, with the 11-minute dry run shown to be irrelevant (2 of those minutes were writing handoff docs).

**Method note worth keeping:** the checkers repeatedly caught the drafts implying an existing client base ("plenty never do", "some owners want this handled"), an unsourced "agencies take six weeks" comparison, "no contract" where a signed one-pager exists, "the pages your business needs" against the five-page cap, and the five-day promise stated without its confirmed-slot qualifier. Assume any new marketing copy drifts the same way and check it the same way.
