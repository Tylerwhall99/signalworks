# Free Website Build — One-Page Agreement

> **DRAFT for attorney review — do not sign clients until reviewed.**
> This is the signable twin of the public /terms/ page. Keep the two in
> sync: if one changes, change the other. Fill the bracketed blanks after
> the entity decision (launch checklist §0/§2).

---

**Between:** [LEGAL ENTITY / YOUR NAME], doing business as Signalworks
("we") and ______________________________ ("you"), for the business
______________________________.

**Date:** ____________

## What we're agreeing to

1. **The build is free.** We design, write, and build a website for your
   business at no charge: the pages your business needs, copy written
   from your intake answers, mobile-first layout, click-to-call, a
   contact form that reaches your inbox, on-site functionality where it
   helps (quote forms, booking buttons), basic on-page SEO, and a Google
   Business Profile connection. First working draft within five business
   days of receiving your intake.

2. **One revision round is included.** A revision is a change of mind —
   structure, layout, what the site does. Further revision rounds, new
   pages after launch, and new content are quoted in writing before any
   work starts, and you can always say no.

3. **Bugs are ours, for 60 days.** If something we built doesn't work,
   that's a bug, not a revision. We fix bugs free for 60 days after
   launch, and a bug fix never uses your revision round.

4. **You own it.** The domain is registered in your own account from day
   one. At launch we transfer the site's code and hosting into accounts
   in your name — or host it for you on the Care Plan ($150/month,
   cancel anytime; if you cancel, we complete the full handoff within 5
   business days at no charge). Your choice, changeable later.

5. **Hosting costs are real and yours.** The build is free; running any
   website costs hosting — roughly $0–20/month if you run it yourself,
   or the Care Plan above if we handle hosting, updates, and support.

6. **Your content is yours to give.** You confirm you have the right to
   use the photos, logos, and any testimonials you provide. We only
   publish testimonials you have written permission to use.

7. **Our record of the work.** We keep an archived copy of the finished
   site and may show it in our portfolio. Initial here to opt out: ____

8. **Either of us can walk away before launch.** If we're not a fit, you
   keep whatever has been delivered to you and owe nothing.

**Not included free** (quoted plainly if you want them): standalone web
apps (booking systems, portals, internal tools), ongoing edits and new
content, photography and logo design, and ongoing growth services.

| | |
| --- | --- |
| Signalworks: ______________________ | Client: ______________________ |
| Date: ______________________ | Date: ______________________ |

> Attorney to add/confirm: governing law (California), limitation of
> liability, and any entity-specific language once the LLC/sole-prop
> decision is made.
